...
<span class="badge badge-soft"><?= $environment === 'live' ? '운영' : '검수' ?> · <?= $settings['enabled'] ? '발송 허용' : '발송 정지' ?></span>
...
<?php if (!$ready): ?>
<section class="card card-body extension-panel"><h2 class="card-title">데이터 준비</h2><p>처음 사용하거나 패키지를 갱신했다면 알림톡 데이터를 설치·갱신해 주세요. 기존 발송 데이터는 보존합니다.</p>
<form method="post" action="<?= $this->e($base) ?>/plugins/bizppurio/settings"><input type="hidden" name="csrf_token" value="<?= $this->e($csrf_token) ?>"><input type="hidden" name="environment" value="<?= $this->e($environment) ?>"><button class="btn btn-primary" name="action" value="install">데이터 설치·갱신</button></form></section>
<?php else: ?>
<section class="card card-body extension-panel"><h2 class="card-title">계정과 발신프로필</h2><p>비즈뿌리오에 등록한 계정 유형과 승인된 발신프로필을 입력해 주세요. 설정 저장 시 인증 확인과 발송 허용이 해제됩니다.</p>
...
<label class="extension-label" for="senderkey">발신프로필 키</label><div class="password-field"><input class="input input-bordered input-block" type="password" id="senderkey" name="senderkey" maxlength="40" required autocomplete="off" spellcheck="false" value="<?= $this->e($settings['senderkey'] ?? '') ?>">
...
<label class="extension-label" for="from">발신번호</label><input class="input input-bordered input-block" id="from" name="from" inputmode="numeric" maxlength="16" required value="<?= $this->e($settings['from'] ?? '') ?>">
<label class="extension-label" for="test_phone">테스트 수신번호</label><input class="input input-bordered input-block" id="test_phone" name="test_phone" inputmode="tel" required value="<?= $this->e($settings['test_phone'] ?? '') ?>">
<label class="extension-label" for="test_only">발송 대상 범위</label><select class="select select-bordered select-block" id="test_only" name="test_only"><option value="1"<?= ($settings['test_only'] ?? true) ? ' selected' : '' ?>>테스트 번호만</option><option value="0"<?= !($settings['test_only'] ?? true) ? ' selected' : '' ?>>입력한 국내 휴대폰 번호</option></select>
<label class="extension-label" for="webhook_ips">업체가 확인한 결과 송신 IP (선택)</label><textarea class="textarea textarea-bordered textarea-block" id="webhook_ips" name="webhook_ips" rows="2"><?= $this->e(implode(' ', $settings['webhook_ips'] ?? [])) ?></textarea><small>공백으로 구분합니다. 프록시를 사용하는 서버는 README의 IP 처리 안내를 확인해 주세요.</small><br>
<div class="card-actions form-actions"><button class="btn btn-primary" name="action" value="save">설정 저장</button></div></form></section>
<?php if ($settings['configured']): ?><section class="card card-body extension-panel"><h2 class="card-title">연결과 발송 상태</h2><p>API 인증: <strong><?= !empty($settings['api_verified']) ? '확인 완료' : '확인 필요' ?></strong></p><p>인증 연결 확인은 발송을 정지하고 이 환경에 저장된 계정 ID와 모듈 비밀번호로 다시 인증합니다. 입력값을 바꿨다면 먼저 설정을 저장해 주세요. 발신프로필 키는 토큰 인증에 사용하지 않으며 메시지는 보내지 않습니다.</p>
...
<?php $this->stop() ?>
<?php $this->start('scripts') ?><?php $this->insert('_password_toggle') ?><?php $this->stop() ?>
