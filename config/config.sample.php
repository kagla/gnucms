...
return [
    ...
    // 세션·메일·알림톡·결제 설정 및 쇼핑몰 배송정보 암호화에 쓰는 시크릿. 32바이트 이상 임의 문자열. 설치기가 만들어 준다.
    // 비즈뿌리오 계정은 플러그인 설정 화면에 입력한다. 이 파일에 업체 비밀번호를 추가하지 않는다.
    // 결제는 PortOne V2 상점·채널·API/웹훅 시크릿을 각 결제 플러그인 설정에서 입력한다.
    // 쇼핑몰 결제 복귀·웹훅 URL에는 위 app.url을 사용한다. HTTPS와 하위 설치 경로를 포함한다.
    'auth' => [
