...
작성일: 2026-09-06. 상태: 1차 구현·모의 API 검증 완료. 실계정 수신 검증은 정식 릴리스 전 별도 필요.
...
- 회귀 테스트 대상은 SQLite와 MySQL/MariaDB다.
- 웹발송 계정의 인증 확인 전 차단, 인증 캐시를 우회한 재확인, 인증 거부 시 발송 정지,
  설정 변경·복원 후 확인값 해제, 웹발송 화면과 사이트 진입을 검증한다.
- 확장 설치·재실행·부분 실패, 접두사, 모의 인증·발송·웹훅, 관리자 권한·CSRF·본문 변조를 검증한다.
- SQLite 복원과 MySQL/MariaDB 네이티브 백업에 확장 테이블을 포함하는 경로를 검증한다.
- 실제 API 인증·과금·카카오 수신·업체 웹훅 ACK 계약은 계정 준비 후 별도로 확인해야 한다.
...
| `src/Maintenance/BackupManager.php` | MySQL/MariaDB 덤프는 `Schema::TABLES`만 포함 | 별도 확장 테이블은 현재 백업에서 누락 |
...
- SQLite/MySQL/MariaDB와 DB prefix를 지원한다. MySQL DDL의 암묵 커밋을 고려해
...
- MySQL/MariaDB는 코어 테이블과 레지스트리의 실제 확장 테이블을 함께 덤프한다.
...
- 복구는 우선 기존 지원 범위인 SQLite에서 검증하고 MySQL/MariaDB는 덤프와 수동 복원 절차를 검증한다.
  이번 기능으로 MySQL/MariaDB 웹 자동 복원까지 지원한다고 표시하지 않는다.
...
| 2 | 확장 스키마·백업 기반 | 지원 DB 설치/재실행/중간 실패/백업, API 1 확장 호환 |
...
- DB: `connectionProvider`로 지원 DB·prefix·UNIQUE 경쟁·설치/갱신 재실행·중간 실패 후 복구.
- 백업: OFF/파일 누락 확장 포함·불완전 설치 검출·SQLite 복원·MySQL/MariaDB 덤프 대상·발송 재개 차단.
