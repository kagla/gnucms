...
모듈 목록의 **관리자 테스트**로 미사용 상태에서 확인하거나, 모듈을 켜고 실행 주소 링크로 실행할 수 있다.
...
탐색 순서는 선택 테마의 `extensions/<이름>/`, 패키지 템플릿, 사이트 공통 템플릿, 코어의 `src/Extension/templates/` 공통 확장 조각이다. 기존 테마에 `admin/extension`이 없어도 코어 조각이 해당 테마의 `admin/layout`을 사용한다. 배치용 `extensions.css`는 테마 자산으로 불러오며 선택 테마에 없으면 기본 자산을 사용한다. 화면 전체를 복사하지 않아도 개별 조각만 재정의할 수 있다. 제공하는 이름은 `bizppurio`, `demo-message`, `demo-reservation`이다. 예를 들어 `extensions/bizppurio/settings.php`를 재정의할 수 있다. 공통 관리자 레이아웃을 재정의하는 테마는 `admin/extension`이 사용하는 블록을 함께 유지한다.
...
- 선택 필드 `entry_path`에 `/preview`처럼 패키지의 고정 실행 경로를 지정하면 관리 목록에
  `{admin_route_prefix 또는 route_prefix}{entry_path}` 주소가 표시된다. `route_prefix`를 생략하면 기존 `/{종류}/{id}`를 사용한다. 사용 중에는 주소 링크와 새 창 아이콘을 제공한다. 플러그인 목록에는 **바로가기** 버튼도 표시한다.
  외부 URL·쿼리·동적 경로는 허용하지 않는다.
- 선택 필드 `public_path`에 `/catalog`처럼 공개 GET 경로를 지정하면 활성화·등록에 성공한 패키지의 사용자 화면을 사이트 상단·모바일 메뉴와 관리자 목록에서 연결한다. 등록된 GET 경로가 없으면 표시하지 않는다. 외부 URL·쿼리·동적 경로는 허용하지 않으며 공개 화면의 권한 검사는 패키지가 담당한다. 테마에서는 `public_extensions`의 패키지 키별 `name`, `url`(사용자 진입 주소), `base_url`(사용자 기본 주소), `admin_url`(관리자 진입 주소, `entry_path`가 있는 경우)을 사용할 수 있다.
- 선택 필드 `route_prefix`로 `/shop` 같은 짧은 기본 주소를 지정한다. 영문 소문자로 시작하는 한 경로 조각만 허용하고 시스템·정적 자산 경로는 예약한다. 코어 또는 먼저 등록한 다른 확장의 경로·콜백과 겹치면 해당 패키지 실행을 막고 관리자에 오류를 표시한다. 기존 `/{종류}/{id}/...` 주소도 같은 처리기로 연결해 이전 링크·POST·콜백을 보존한다. 패키지 키와 DB·의존성·폴더 이름은 바꾸지 않는다.
- 선택 필드 `admin_route_prefix`로 `/admin/shop` 같은 관리자 전용 기본 주소를 지정한다. `admin: true`인 라우트와 `entry_path`에 사용하며 공개 라우트·결제 콜백의 주소에는 영향을 주지 않는다. 코어·다른 패키지 경로와 충돌하면 실행을 막는다.
...
- 경로 앞에 `route_prefix`가 자동으로 붙는다. 생략하면 `/{plugins 또는 modules}/{id}`를 사용한다.
- 패키지에서 주소를 만들 때는 `$context->path('/cart')` 또는 `$context->routePrefix`를 사용하고 하위 디렉터리 설치의 base path를 앞에 붙인다. `route('GET', '/', ...)`는 기본 주소와 끝에 `/`가 붙는 주소 모두에 연결한다.
...
- `admin: true`는 전체 관리자 권한을 검사한다. `admin_route_prefix`가 있으면 그 아래에 등록하고 기존 사용자 기본 주소·패키지 주소도 같은 검사로 연결한다. 필요한 경우 `legacyPath`로 이전 상대 경로를 지정할 수 있다. 예: `route('GET', '/', $handler, admin: true, legacyPath: '/admin')`은 관리자 루트와 이전 `/shop/admin`, `/modules/shop/admin`을 연결한다. 기본값은 공개 라우트다.
...
웹 ZIP 설치·자동 업데이트·의존 버전 범위 검사·패키지 설정/임의 메뉴 등록 API·
...
실제 구현 예시는 [비즈뿌리오 플러그인](../plugins/bizppurio/README.md)을 참고한다.
이를 사용하는 비즈뿌리오 알림톡 운영 모듈은 `feat/bizppurio-messaging` 브랜치에 별도로 보관한다.
